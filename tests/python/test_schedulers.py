...
def test_add_sequence(mock_cpp_engine, base_config):
    ...
    seq_id = scheduler.add_sequence("req-1", prompt_token_ids=[1, 2, 3, 4], max_new_tokens=10)
...
def test_prefill_and_decode_cycle(mock_cpp_engine, base_config):
    ...
    # Add 2 requests
    scheduler.add_sequence("req-1", prompt_token_ids=[10] * 32, max_new_tokens=2) # Needs 2 blocks
    scheduler.add_sequence("req-2", prompt_token_ids=[20] * 16, max_new_tokens=2) # Needs 1 block
...
def test_vram_oom_preemption(mock_cpp_engine, base_config):
    ...
    seq_id = scheduler.add_sequence("req-oom", prompt_token_ids=[1] * 16, max_new_tokens=10)
...
def test_sla_priority_sorting(mock_cpp_engine, dynamic_config):
    ...
    scheduler.add_dynamic_sequence(
        "req-low", prompt_token_ids=[1] * 10, max_new_tokens=5, priority=PriorityLevel.LOW
    )
    # Add high-priority request second
    scheduler.add_dynamic_sequence(
        "req-critical", prompt_token_ids=[2] * 10, max_new_tokens=5, priority=PriorityLevel.CRITICAL
    )
...
def test_prefix_caching_hit(mock_cpp_engine, dynamic_config):
    ...
    seq1_id = scheduler.add_dynamic_sequence("req-1", prompt_token_ids=shared_prompt, max_new_tokens=1)
    scheduler.schedule()
    ...
    seq2_id = scheduler.add_dynamic_sequence("req-2", prompt_token_ids=shared_prompt, max_new_tokens=5)
...
def test_smart_victim_preemption(mock_cpp_engine, dynamic_config):
    ...
    scheduler.add_dynamic_sequence("req-high", prompt_token_ids=[1]*16, max_new_tokens=10, priority=PriorityLevel.HIGH)
    scheduler.add_dynamic_sequence("req-low", prompt_token_ids=[2]*16, max_new_tokens=10, priority=PriorityLevel.LOW)

...
from nexuscache.server.pipeline_manager import ExecutionPipelineManager, AdaptiveBatchTuner
...
def dummy_model_forward(input_ids: torch.Tensor, slot_mapping: torch.Tensor) -> torch.Tensor:
    """Mock PyTorch forward pass returning logits tensor on CUDA."""
    batch_size = input_ids.shape[0]
    return torch.randn((batch_size, 32000), dtype=torch.float16, device=input_ids.device)
...
class TestPipelineManager:
    ...
    def test_adaptive_batch_tuner_scaling(self):
        ...
        tuner = AdaptiveBatchTuner(initial_token_budget=2048, min_token_budget=1024, max_token_budget=8192)

        # Force 50 iterations with low step time + high free VRAM -> budget should scale UP
        for _ in range(50):
            budget = tuner.update_budget(current_batch_size=16, step_duration_ms=2.0, free_vram_ratio=0.50)

        assert budget > 2048

        # Force 50 iterations with high VRAM pressure -> budget should scale DOWN
        for _ in range(50):
            budget = tuner.update_budget(current_batch_size=64, step_duration_ms=15.0, free_vram_ratio=0.05)
    ...
    def test_pipeline_execution_and_cuda_graphs(self, cpp_subsystem, allocator_config):
        ...
        sched_config = SchedulerConfig(max_num_batched_tokens=512, max_num_seqs=16, max_paged_blocks=32, block_size=16)
        ...
        s1 = scheduler.add_sequence("req_1", [10, 11, 12, 13], max_new_tokens=5)
        s2 = scheduler.add_sequence("req_2", [20, 21], max_new_tokens=5)

        # 1. Run Prefill Batch
        batch_prefill = scheduler.schedule()
        results_prefill = pipeline.execute_batch_step(batch_prefill, scheduler, dummy_model_forward)
        ...
        results_decode = pipeline.execute_batch_step(batch_decode, scheduler, dummy_model_forward)

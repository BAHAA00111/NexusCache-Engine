...
class FastLoadGenerator:
    def __init__(
        ...
        base_prompt = prompt_prefix + (" token" * max(0, input_token_len - len(prompt_prefix.split())))
    ...
    async def _send_request(self, session: aiohttp.ClientSession, req_id: str) -> RequestResult:
        ...
        async with self.semaphore:
            try:
                if self.stream:
                    async with session.post(self.target_url, json=self.payload) as response:
                        ...
                        if response.status == 200:
                            async for line_bytes in response.content:
                                ...
                                if first_token_time is None:
                                    ...
                                    result.itl_ms.append((chunk_now - last_chunk_time) * 1000.0)
                    ...
                    async with session.post(self.target_url, json=self.payload) as response:
                        ...
                        if response.status == 200:
                            ...
                            result.gen_tokens = data.get("usage", {}).get("completion_tokens", self.max_output_tokens)
                        else:
                            result.error_msg = f"HTTP {response.status}"
...
def print_summary_table(summary: BenchmarkSummary) -> None:
    table = Table(title="NexusCache Load Generator - Benchmark Results", show_header=True, header_style="bold cyan")
    table.add_column("Metric", style="dim", width=30)
    table.add_column("Value", justify="right", style="bold green")

    table.add_row("Total Duration", f"{summary.duration_sec:.2f} s")
    table.add_row("Total / Successful Requests", f"{summary.total_requests} / {summary.successful_requests}")
    table.add_row("Failed Requests", f"{summary.failed_requests}", style="bold red" if summary.failed_requests > 0 else "dim")
...
def main():
    ...
    parser.add_argument("--url", type=str, default="http://localhost:8000/v1/chat/completions", help="Target API endpoint URL")
    parser.add_argument("--qps", type=float, default=0.0, help="Target QPS (0 = unlimited max throughput speed)")
    parser.add_argument("--num-requests", type=int, default=100, help="Total requests to fire")
    parser.add_argument("--concurrency", type=int, default=32, help="Max parallel connections")
    parser.add_argument("--uniform", action="store_true", help="Use uniform pacing instead of Poisson process")
    parser.add_argument("--no-stream", action="store_true", help="Disable response streaming")
    parser.add_argument("--prompt-tokens", type=int, default=128, help="Prompt length in tokens")
    parser.add_argument("--output-tokens", type=int, default=128, help="Max generation tokens")
    parser.add_argument("--output-json", type=str, default=None, help="Save summary to JSON file")

    args = parser.parse_args()

    console.print(f"[bold green]Starting benchmark on[/bold green] [yellow]{args.url}[/yellow] "
    ...
    if summary.failed_requests > 0:
        ...
        errors = list(set(r.error_msg for r in generator.results if not r.success))
        for err in errors[:5]:
            console.print(f"{err}")

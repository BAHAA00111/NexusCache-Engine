...
class Sequence:
    ...
    def is_finished(self) -> bool:
        return len(self.generated_token_ids) >= self.max_new_tokens or self.state == SequenceState.FINISHED


@dataclass
class SchedulerConfig:
    """Runtime limits and capacity bounds for continuous batching."""
    max_num_batched_tokens: int = 4096   # Max total tokens across prefill + decode in 1 step
...
class Scheduler:
    ...
    def add_sequence(self, request_id: str, prompt_token_ids: List[int], max_new_tokens: int) -> int:
        """Adds a new client request into the scheduler waiting queue."""
        seq_id = hash(request_id) & 0x7FFFFFFF  # Positive 32-bit integer for C++ compatibility
        seq = Sequence(
            seq_id=seq_id,
        ...
        logger.debug(f"[Scheduler] Added seq_id={seq_id} (request_id={request_id}) to WAITING queue.")
        return seq_id
    ...
    def schedule(self) -> BatchOutput:
        ...
        for seq in self.running_queue:
            ...
            if needs_new_block and num_free_blocks < 1:
                ...
                logger.warning(f"[Scheduler] Preempting seq_id={seq.seq_id} due to KV-cache exhaustion.")
        ...
        while self.waiting_queue and len(self.running_queue) + len(prefill_batch) < self.config.max_num_seqs:
    ...
    def _schedule_preempted(
        ...
        while self.preempted_queue and len(self.running_queue) + len(prefill_batch) < self.config.max_num_seqs:
    ...
    def _free_sequence_resources(self, seq: Sequence) -> None:
        ...
        logger.debug(f"[Scheduler] Reclaimed C++ KV blocks for finished seq_id={seq.seq_id}.")

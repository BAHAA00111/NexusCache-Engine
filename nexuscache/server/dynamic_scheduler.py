...
class DynamicSchedulerConfig(SchedulerConfig):
    ...
    decode_protection_ratio: float = 0.7  # Reserve token budget for decode phase under load
    cpu_swap_space_blocks: int = 2048     # Number of host CPU RAM blocks for swap preemption
    max_paged_blocks: int = 256           # Scaled from 64 to relieve KV-cache block contention
    max_num_batched_tokens: int = 4096    # Expanded token budget for parallel prefill passes
...
class PrefixTrieNode:
    ...
    def __init__(self, block_id: Optional[int] = None):
        self.children: Dict[int, "PrefixTrieNode"] = {}
        self.block_id: Optional[int] = block_id
...
class PrefixCacheManager:
    ...
    def insert_prefix(self, token_ids: List[int], physical_block_ids: List[int]) -> None:
        ...
        for i in range(num_blocks):
            ...
            if block_key not in curr.children:
                curr.children[block_key] = PrefixTrieNode(block_id=physical_block_ids[i])
...
class DynamicScheduler(Scheduler):
    ...
    def __init__(
        ...
        super().__init__(config=config, block_manager=block_manager, page_table=page_table)
    ...
    def add_dynamic_sequence(
        ...
        if self.dynamic_config.enable_prefix_caching:
            matched_tokens, matched_blocks = self.prefix_cache.match_prefix(prompt_token_ids)
    ...
    def schedule(self, sim_time: Optional[float] = None) -> BatchOutput:
        """
        ...
        self.running_queue.sort(
            key=lambda s: (
                s.priority.value if isinstance(s, DynamicSequence) else PriorityLevel.NORMAL.value,
                s.arrival_time,
            )
        )
        ...
        for seq in self.running_queue:
            ...
            if needs_new_block and num_free_blocks < 1:
                victim = self._select_preemption_victim(exclude_seq_id=seq.seq_id, preempted_ids=preempted_ids)
        ...
        self.waiting_queue.sort(
            key=lambda s: (
                s.calculate_scheduling_score(now) if isinstance(s, DynamicSequence) else -s.arrival_time
            ),
            reverse=True,
        )

        idx = 0
        while idx < len(self.waiting_queue) and len(self.running_queue) < self.config.max_num_seqs:
            seq = self.waiting_queue[idx]
            
            effective_prefill_tokens = (
                seq.remaining_prefill_tokens if isinstance(seq, DynamicSequence) else seq.num_prompt_tokens
            )
            ...
            if num_batched_tokens + effective_prefill_tokens > self.config.max_num_batched_tokens:
                ...
                if is_critical and len(prefill_batch) == 0 and num_free_blocks >= required_blocks:
    ...
    def _select_preemption_victim(
        self, exclude_seq_id: int, preempted_ids: Optional[Set[int]] = None
    ) -> Optional[Sequence]:
        """
    ...
    def _free_sequence_resources(self, seq: Sequence) -> None:
        ...
        if isinstance(seq, DynamicSequence) and self.dynamic_config.enable_prefix_caching:
